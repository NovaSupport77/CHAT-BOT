...
OWNER_ID = int(os.environ.get("OWNER_ID", "7589623332")) # Default placeholder
...
# Fixed to load from file if exists, otherwise use the minimal default structure
DATA = {}
try:
    with open("conversation.json", "r", encoding="utf-8") as f:
        DATA = json.load(f)
except (FileNotFoundError, json.JSONDecodeError):
    print("conversation.json not found or corrupted. Using minimal default data.")
    DATA = {
...
async def save_chat_id(chat_id, type_):
    ...
    if chat_id_str not in KNOWN_CHATS[type_]:
        ...
        # Ensure only the OWNER_ID can write this file, or handle concurrent writes.
        # For simplicity in this single-file bot, we write directly.
...
def get_help_main_buttons():
    ...
    # Exclude 'games' category buttons
    return InlineKeyboardMarkup([
        [
...
async def callbacks_handler(client, query):
    ...
    if data == "about":
        ...
        buttons = []
        if category in ["couple", "chatbot", "tools", "group"]: # Add all current categories
...
# -------- New Chat Member Handler (Welcome) --------
@app.on_message(filters.new_chat_members)
async def new_member_handler(client, message):
    if message.from_user.is_self: # Ignore own service messages
        return
    ...
    for new_member in message.new_chat_members:
        if new_member.id == me.id:
            ...
            await message.reply_text("👋 ᴛʜᴀɴᴋs ғᴏʀ ᴀᴅᴅɪɴɢ ᴍᴇ! 𝐈 𝐚𝐦 𝐄𝐯𝐚𝐫𝐚, 𝐲𝐨𝐮𝐫 𝐀𝐝𝐯𝐚𝐧𝐜𝐞𝐝 𝐂𝐡𝐚𝐭 𝐁𝐨𝐭. 𝐔𝐬𝐞 /help 𝐭𝐨 𝐬𝐞𝐞 𝐦𝐲 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬.")
            break
        # Optional: Welcome new members who are not the bot
        # await message.reply_text(f"𝐖𝐞𝐥𝐜𝐨𝐦𝐞 [{new_member.first_name}](tg://user?id={new_member.id}) 𝐭𝐨 𝐭𝐡𝐞 𝐠𝐫𝐨𝐮𝐩!")
...
async def start_cmd(client, message):
    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        # Ding Dong Animation (Simplified/Shortened)
...
async def help_cmd(client, message):
    user = message.from_user
    if message.chat.type == enums.ChatType.PRIVATE:
        # For private chat, send the main help menu as a photo
        me = await app.get_me()
        await message.reply_photo(
            START_PHOTO, # Using start photo for consistency
            caption="📜 𝐂ᴏᴍᴍᴀɴᴅs 𝐌ᴇɴᴜ:\n\n𝐂ʜᴏᴏsᴇ ᴀ ᴄᴀᴛᴇɢᴏʀʏ ʙᴇʟᴏᴡ:",
            reply_markup=get_help_main_buttons(),
            has_spoiler=False # No spoiler for the help menu
        )
    else:
        # For group chat, send the help menu as text
        await message.reply_text(
            "📜 𝐂ᴏᴍᴍᴀɴᴅs 𝐌ᴇɴᴜ:\n\n𝐂ʜᴏᴏsᴇ ᴀ ᴄᴀᴛᴇɢᴏʀʏ ʙᴇʟᴏᴡ:",
            reply_markup=get_help_main_buttons() # Group command sends buttons for interaction
        )
...
async def ping_cmd(client, message):
    start = time.time()
    m = await message.reply_text("Pɪɴɢɪɴɢ...")
    end = time.time()
...
async def tagall_cmd(client, message):
    ...
    for i in range(0, len(member_list), chunk_size):
        ...
        try:
            # Send the tag message (FIXED: Send to the chat)
    ...
    if TAGGING.pop(chat_id, False): # Use pop to safely remove and check status
...
async def stop_tagging_cmd(client, message):
    ...
    if TAGGING.pop(chat_id, False): # Safely check and remove
...
async def staff_cmd(client, message):
    ...
    if owner_user:
        staff_text += f"**𝐎𝐰𝐧𝐞𝐫:** [{owner_user.first_name}](tg://user?id={owner_user.id})\n"
    ...
    if admin_users:
        ...
        if not owner_user:
             staff_text += "❗ 𝐍𝐨 𝐀𝐝𝐦𝐢𝐧𝐢𝐬𝐭𝐫𝐚𝐭𝐨𝐫𝐬 𝐟𝐨𝐮𝐧𝐝."
...
async def botlist_cmd(client, message):
    ...
    try:
        ...
        return await message.reply_text("🚫 𝐂𝐚𝐧𝐧𝐨𝐭 𝐟𝐞𝐭𝐜𝐡 𝐦𝐞𝐦𝐛𝐞𝐫 𝐥𝐢𝐬𝐭 𝐝𝐮𝐞 𝐭𝐨 𝐫𝐞𝐬𝐭𝐫𝐢𝐜𝐭𝐢𝐨𝐧𝐬.")

    await message.reply_text(f"🤖 𝐂𝐮𝐫𝐫𝐞𝐧𝐭𝐥𝐲 **{bot_count}** ʙᴏᴛs ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ.")
...
async def afk_trigger_handler(client, message):
    ...
    if user_id in AFK_USERS:
        ...
        # Check if it's NOT an /afk command (since /afk handles its own return)
    ...
    for afk_user_id in mentions_to_check:
        if afk_user_id in AFK_USERS:
            ...
            time_afk = get_readable_time(int(time.time() - afk_data["time"]))
            afk_mention = f"[{afk_data['first_name']}](tg://user?id={afk_user_id})"

            await message.reply_text(
                f"⚠️ **{afk_mention}** ɪs 𝐀ғᴋ!\n"
                f"𝐑ᴇᴀsᴏɴ: {afk_data['reason']}\n"
                f"𝐓ɪᴍᴇ 𝐀ғᴋ: {time_afk}",
                parse_mode=enums.ParseMode.MARKDOWN
            )
            # Only send one AFK warning per message to avoid spam
            break

# -------- Chatbot Reply Handler (FIXED) --------
@app.on_message(filters.text & filters.private & ~filters.bot)
async def private_chatbot_handler(client, message):
    """Handles chatbot replies in private chat (always active)."""
    text = message.text
...
@app.on_message(filters.text & filters.group & ~filters.command(["chatbot"]) & ~filters.bot)
async def group_chatbot_handler(client, message):
    """
    Handles chatbot replies in group chat only if enabled.
    Replies to all messages (not just replies) with a 25% chance, or
    ...
    # 1. Check if the message is a direct reply to the bot
    if message.reply_to_message and message.reply_to_message.from_user and message.reply_to_message.from_user.id == me.id:
        should_reply = True

    # 2. Check if the bot is mentioned in the message (if the group is not private)
    if message.text and f"@{me.username}" in message.text.lower():
        should_reply = True

    # 3. Random reply chance (25%) for general conversation if not a direct mention/reply
    if not should_reply and random.random() < 0.25:
        should_reply = True
...
if __name__ == "__main__":
    print("Starting bot...")
    # Initialize CHATBOT_STATUS for known groups (default to False if not in data)
    for chat_id_str in KNOWN_CHATS.get("groups", []):
        try:
            chat_id = int(chat_id_str)
            # Assuming default state for all groups is 'disabled' on startup
            # unless a persistent state mechanism is added.
