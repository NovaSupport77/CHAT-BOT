...
def is_chatbot_enabled(_, __, message: Message):
	...
	return False # Should return boolean, not None
...
# -------- Voice Chat Notifications (FIXED FILTER) --------
...
async def voice_chat_started_handler(client, message):
	...
	# Use a clear, formatted message to notify members
	await client.send_message(
		chat_id,
		f"🎙️ 𝐕𝐨𝐢𝐜𝐞 𝐂𝐡𝐚𝐭 𝐒𝐭𝐚𝐫𝐭𝐞𝐝!\n\n"
		f"𝐀 𝐕𝐨𝐢𝐜𝐞 𝐂𝐡𝐚𝐭 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐬𝐭𝐚𝐫𝐭𝐞𝐝 𝐢𝐧 **{title}**. 𝐂𝐨𝐦𝐞 𝐣𝐨𝐢𝐧 𝐭𝐡𝐞 𝐜𝐨𝐧𝐯𝐞𝐫𝐬𝐚𝐭𝐢𝐨𝐧! 🎧",
		parse_mode=enums.ParseMode.MARKDOWN
	)
...
async def voice_chat_ended_handler(client, message):
	...
	# Extract duration from the update object
	duration_sec = message.voice_chat_ended.duration if message.voice_chat_ended else 0
	readable_duration = get_readable_time(duration_sec)
	
	await client.send_message(
		chat_id,
		f"⏹️ 𝐕𝐨𝐢𝐜𝐞 𝐂𝐡𝐚𝐭 𝐄𝐧𝐝𝐞𝐝!\n\n"
		f"𝐓𝐡𝐞 𝐕𝐨𝐢𝐜𝐞 𝐂𝐡𝐚𝐭 𝐞𝐧𝐝𝐞𝐝 𝐚𝐟𝐭𝐞𝐫 {readable_duration}. 𝐆𝐨𝐨𝐝𝐛𝐲𝐞 𝐟𝐨𝐫 𝐧𝐨𝐰! 👋",
		parse_mode=enums.ParseMode.MARKDOWN
	)
...
async def start_cmd(client, message):
	...
	if message.chat.type == enums.ChatType.PRIVATE:
		...
		await message.reply_photo(
			...
			reply_markup=get_start_buttons(me.username),
			parse_mode=enums.ParseMode.MARKDOWN,
			# FIX: Added spoiler tag as requested
			has_spoiler=True
		)
...
async def id_cmd(client, message):
	...
	# Ensure all users are tagged correctly, even if they don't have a username
...
async def tagall_cmd(client, message):
	...
	if len(message.command) > 1:
		...
		# Preview of replied message text (up to 50 chars)
	...
	try:
		# Filter out bots and deleted accounts during collection
		async for member in app.get_chat_members(chat_id):
			if not (member.user.is_bot or member.user.is_deleted):
				member_list.append(member.user)
	except Exception:
		TAGGING[chat_id] = False
		return await m.edit_text("🚫 𝐄𝐫𝐫𝐨𝐫 𝐢𝐧 𝐟𝐞𝐭𝐜𝐡𝐢𝐧𝐠 𝐦𝐞𝐦𝐛𝐞𝐫s: 𝐌𝐚𝐲𝐛𝐞 𝐭𝐡𝐢𝐬 𝐠𝐫𝐨𝐮𝐩 𝐢𝐬 𝐭𝐨𝐨 𝐛𝐢𝐠 𝐨𝐫 𝐈 𝐝𝐨𝐧't 𝐡𝐚𝐯𝐞 𝐩𝐞𝐫𝐦𝐢𝐬𝐬𝐢𝐨𝐧s.")
...
async def couples_cmd(client, message):
	member_list = []
	try:
		# Use only non-bot, non-deleted accounts
...
async def love_cmd(client, message):
	...
	# FIX: Added missing love_percent calculation
	love_percent = random.randint(30, 99)

	text = f"❤️ 𝐋ᴏᴠᴇ 𝐏ᴏssɪʙʟɪᴛʏ\n" \
			f"{names[0]} & {names[1]}’𝐬 ʟᴏᴠᴇ ʟᴇᴠᴇʟ ɪs {love_percent}% 😉"
			
	# FIX: Corrected InlineKeyboardMarkup syntax (was "eyboardMarkup")
	buttons = InlineKeyboardMarkup([[InlineKeyboardButton("𝐒ᴜᴘᴘᴏʀᴛ", url=SUPPORT_CHAT)]])
	await message.reply_text(text, reply_markup=buttons)

# -------- /afk Command (FIXED LOGIC) --------
@app.on_message(filters.command("afk"))
async def afk_cmd(client, message):
	user_id = message.from_user.id
	user_name = message.from_user.first_name
	
	# 1. Check if the user is already AFK (meaning they are coming back)
...
# -------- /mmf Command (Disabled Placeholder) --------
# The decorator is commented out as the feature is noted as unavailable/disabled
# @app.on_message(filters.command("mmf") & filters.group)
async def mmf_cmd(client, message):
	# This feature requires complex image processing libraries which are often difficult
	# to set up in cloud environments. It is intentionally disabled for stability.
...
# -------- AFK Mention Check Handler (MISSING, ADDED HERE) --------
@app.on_message(filters.text & filters.group & ~filters.me, group=1)
async def afk_mention_handler(client, message: Message):
	# Check if the message is a command or a reply to the bot itself
	if message.text and message.text.startswith('/'):
		return

	user_id = message.from_user.id
	
	# Check if the user is returning from AFK by sending a regular message
	if user_id in AFK_USERS:
		afk_data = AFK_USERS.pop(user_id)
		time_afk = get_readable_time(int(time.time() - afk_data["time"]))
		await message.reply_text(
			f"𝐘ᴇᴀʜ, [{afk_data['first_name']}](tg://user?id={user_id}), ʏᴏᴜ 𝐚𝐫𝐞 ʙᴀᴄᴋ, ᴏɴʟɪɴᴇ! (𝐀ғᴋ ғᴏ𝐫: {time_afk}) 😉",
			parse_mode=enums.ParseMode.MARKDOWN
		)
		return
	...
	# 1. Check for mentions/replies in the message entities
	if message.entities:
	...
	# 3. Check for AFK user mentions in raw text (e.g. @username)
	if message.text:
		for uid, data in AFK_USERS.items():
			if data.get('username') and f"@{data['username']}" in message.text:
				mentioned_ids.add(uid)


	for afk_user_id in mentioned_ids:
		data = AFK_USERS.get(afk_user_id)
		if data:
			time_afk = get_readable_time(int(time.time() - data["time"]))
			
			# Construct the AFK response
			afk_response = (
				f"😴 [{data['first_name']}](tg://user?id={afk_user_id}) 𝐢𝐬 𝐀ғᴋ!\n"
				f"⌚ 𝐋𝐚𝐬𝐭 𝐬𝐞𝐞𝐧: {time_afk} 𝐚𝐠𝐨\n"
				f"✍️ 𝐑𝐞𝐚𝐬𝐨𝐧: {data['reason']}"
			)
			# Send the notification as a reply to the original message
			await message.reply_text(afk_response, parse_mode=enums.ParseMode.MARKDOWN)

# -------- General Chatbot Reply Handler (MISSING, ADDED HERE) --------
# Priority group=2 (runs after AFK check)
@app.on_message(
	filters.text & 
	(filters.private | (filters.group & (filters.mentioned | filters.reply | is_chatbot_enabled))) & 
	~filters.via_bot & ~filters.command & ~filters.me, 
	group=2
)
async def chat_reply_handler(client, message: Message):
	# If in a group, only proceed if chatbot is enabled OR the bot is mentioned/replied to.
	is_group = message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]
	
	if is_group and not (message.reply_to_message or filters.mentioned(message)):
		if not is_chatbot_enabled(client, message.chat, message):
			return
	
	# Get the text to process
	# If it's a reply to the bot, strip the bot's mention from the text
	text_to_process = message.text
	if message.reply_to_message and message.reply_to_message.from_user.is_self:
		if message.text and message.text.startswith(f"@{client.me.username}"):
			text_to_process = message.text.replace(f"@{client.me.username}", "", 1).strip()
	...
	if response:
		try:
			...
			print(f"Error sending reply (sticker/text): {e}")
			# Fallback to plain text if sticker sending fails
			if is_sticker:
				await message.reply_text(f"I wanted to send a sticker, but failed. Here's a text reply instead: {random.choice(DATA.get('daily', ['Hello 👋']))}")


# -------- /dice, /jackpot, etc., Games Commands (MISSING, ADDED HERE) --------

@app.on_message(filters.command("dice"))
async def dice_game_cmd(client, message):
	await message.reply_dice(emoji=enums.DiceEmoji.DICE)

@app.on_message(filters.command("jackpot"))
async def jackpot_game_cmd(client, message):
	await message.reply_dice(emoji=enums.DiceEmoji.SLOT_MACHINE)

@app.on_message(filters.command("football"))
async def football_game_cmd(client, message):
	await message.reply_dice(emoji=enums.DiceEmoji.FOOTBALL)

@app.on_message(filters.command("basketball"))
async def basketball_game_cmd(client, message):
	await message.reply_dice(emoji=enums.DiceEmoji.BASKETBALL)

@app.on_message(filters.command("bowling"))
async def bowling_game_cmd(client, message):
	await message.reply_dice(emoji=enums.DiceEmoji.BOWLING)

# -------- /staff Command (MISSING, ADDED HERE) --------
	...
	try:
		async for member in app.get_chat_members(message.chat.id):
			if member.status in [enums.ChatMemberStatus.OWNER, enums.ChatMemberStatus.ADMINISTRATOR]:
				# Exclude the bot from the staff list
				if not member.user.is_self:
					user_mention = f"[{member.user.first_name}](tg://user?id={member.user.id})"
					role = "👑 𝐎𝐰𝐧𝐞𝐫" if member.status == enums.ChatMemberStatus.OWNER else "🛠️ 𝐀𝐝𝐦𝐢𝐧"
					staff_members.append(f"{role}: {user_mention}")
	except Exception:
		return await message.reply_text("🚫 𝐂𝐚𝐧𝐧𝐨𝐭 𝐟𝐞𝐭𝐜𝐡 𝐬𝐭𝐚𝐟𝐟 𝐦𝐞𝐦𝐛𝐞𝐫𝐬 𝐝𝐮𝐞 𝐭𝐨 𝐫𝐞𝐬𝐭𝐫𝐢𝐜𝐭𝐢𝐨𝐧s.")
	
	if not staff_members:
		return await message.reply_text("❗ 𝐍𝐨 𝐚𝐝𝐦𝐢𝐧𝐬 𝐟𝐨𝐮𝐧𝐝 𝐢𝐧 𝐭𝐡𝐢𝐬 𝐠𝐫𝐨𝐮𝐩.")
...
# -------- /botlist Command (MISSING, ADDED HERE) --------
...
async def botlist_cmd(client, message):
	...
	try:
		...
		return await message.reply_text("🚫 𝐄𝐫𝐫𝐨𝐫 𝐢𝐧 𝐟𝐞𝐭𝐜𝐡𝐢𝐧𝐠 𝐦𝐞𝐦𝐛𝐞𝐫s 𝐝𝐮𝐞 𝐭𝐨 𝐫𝐞𝐬𝐭𝐫𝐢𝐜𝐭𝐢𝐨𝐧s.")

	bot_list_text = "\n".join(bots)
	
	await message.reply_text(
		f"🤖 𝐁𝐨𝐭𝐬 𝐢𝐧 𝐭𝐡𝐢𝐬 𝐆𝐫𝐨𝐮𝐩: {bot_count}\n\n"
		f"**𝐋𝐢𝐬𝐭 𝐨𝐟 𝐁𝐨𝐭𝐬:**\n{bot_list_text}",
		parse_mode=enums.ParseMode.MARKDOWN
	)
