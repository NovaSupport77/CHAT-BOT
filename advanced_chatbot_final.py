...
OWNER_ID = int(os.environ.get("OWNER_ID", "7589623332")) # Please ensure this is your actual Telegram User ID
...
HELP_COMMANDS_TEXT_MAP = {
    ...
    "tools": (
        ...
        "\n_𝐓ᴀɢᴀʟʟ/𝐒ᴛᴏᴘ/𝐁ᴏᴛʟɪsᴛ ʀᴇǫᴜɪʀᴇs 𝐀ᴅᴍɪɴ. 𝐎ᴛʜᴇʀs ᴀʀᴇ ғᴏʀ ᴇᴠᴇʀʏᴏɴᴇ."
    ),
    "games": (
        "📜 𝐆ᴧᴍᴇs 𝐂ᴏᴍᴍᴧɴᴅs:\n"
        "/dice ~ 𝐑ᴏʟʟ ᴧ ᴅɪᴄᴇ (🎲)\n"
...
# Loads the conversation.json file for smart replies
try:
    with open("conversation.json", "r", encoding="utf-8") as f:
        DATA = json.load(f)
except Exception:
    # If the JSON file is missing or fails to load, use the minimal default
    DATA = {
...
def get_reply(text: str):
    ...
    Implements the 80% Text / 20% Sticker split.
    ...
    for word, cat in KEYWORDS.items():
        ...
        if word in text:
            ...
            # --- 80/20 Logic ---
            # 20% chance to send a sticker if it's a sticker category and list is not empty
            if is_sticker_category and DATA.get(cat) and random.random() < 0.20:
                return (random.choice(DATA[cat]), True) 
            
            # 80% chance to send text reply (or if sticker chance failed)
            # Map the sticker category back to its text category (e.g., sticker_cute -> cute)
            text_cat = cat if not is_sticker_category else cat.replace("sticker_", "")
            
            if text_cat in DATA and DATA[text_cat]:
                return (random.choice(DATA[text_cat]), False)
    
    # If no specific keyword is found, send a general/daily reply (Always text)
    return (random.choice(DATA.get("daily", ["Hello 👋"])), False)
...
# -------- Inline Button Handlers & Menus (Logic is fine, no changes needed) --------
...
# -------- /developer Command (Logic is fine) --------
...
async def broadcast_cmd(client, message):
    ...
    for chat_type in ["privates", "groups"]:
        for chat_id_str in KNOWN_CHATS[chat_type]:
            ...
            try:
                if content_to_send:
                    # Use client.copy_message (FIXED: Corrected call)
                    await client.copy_message(chat_id, message.chat.id, content_to_send.id) 
                elif text:
                    # Use client.send_message (FIXED: Corrected call)
                    await client.send_message(chat_id, text) 
                sent += 1
            except Exception as e: # FIXED SYNTAX: The try-except block was broken
...
async def tagall_cmd(client, message):
    ...
    try:
        # Use iter_chat_members() with type filtering for efficiency and correctness (FIXED: Iteration)
    ...
    for i in range(0, len(member_list), chunk_size):
        ...
        try:
            # Send the tag message (FIXED: Corrected call)
...
@app.on_message(filters.command("stop") & filters.group) # FIXED: Added the missing decorator
async def stop_tagging_cmd(client, message):
    ...
    if TAGGING.get(chat_id): # FIXED LOGIC: Checks the dictionary for active tagging
        TAGGING[chat_id] = False
...
async def staff_cmd(client, message):
    staff_list = []
    # Get group owner ID to distinguish
    chat = await client.get_chat(message.chat.id)
    owner_id = chat.owner_id if hasattr(chat, 'owner_id') else None
    
    async for member in app.get_chat_members(message.chat.id):
        if member.status in [enums.ChatMemberStatus.OWNER, enums.ChatMemberStatus.ADMINISTRATOR] and not member.user.is_bot:
            staff_list.append(member.user)

    if not staff_list:
        return await message.reply_text("𝐍ᴏ ᴀᴅᴍɪɴs/ᴏᴡɴᴇʀs ғᴏᴜɴᴅ.")

    text = "👑 **𝐆ʀᴏᴜᴘ 𝐒ᴛᴀғғ 𝐌ᴇᴍʙᴇʀs:** 🛡️\n"
    for user in staff_list:
        status_emoji = "👑" if user.id == owner_id else "🛡️"
        text += f"{status_emoji} [{user.first_name}](tg://user?id={user.id})\n"
...
async def botlist_cmd(client, message):
    ...
    if not bot_list:
        return await message.reply_text("✅ 𝐍ᴏ 𝐛ᴏᴛs 𝐟ᴏᴜɴᴅ ɪɴ ᴛʜɪs 𝐠ʀᴏᴜᴘ.")

    text = f"🤖 **𝐁ᴏᴛ 𝐋ɪsᴛ ({len(bot_list)} 𝐁ᴏᴛs 𝐅ᴏᴜɴᴅ):**\n"
    for user in bot_list:
        username_str = f"`@{user.username}`" if user.username else "𝐍/𝐀"
        text += f"• [{user.first_name}](tg://user?id={user.id}) - {username_str}\n"
...
async def couples_cmd(client, message):
    member_list = []
    try:
        # Use iter_chat_members() with type filtering for efficiency and correctness
        async for member in app.get_chat_members(message.chat.id):
            if not (member.user.is_bot or member.user.is_deleted):
                member_list.append(member.user)
    except Exception:
...
async def cute_cmd(client, message):
    ...
    await message.reply_text(text, reply_markup=buttons, parse_mode=enums.ParseMode.MARKDOWN) # FIXED missing text and reply_markup
...
async def love_cmd(client, message):
    ...
    # Split the names (FIXED: Corrected list comprehension syntax)
    names = [n.strip() for n in message.text.split(None, 1)[1].split('+')]
...
async def afk_trigger_handler(client, message):
    ...
    if user_id in AFK_USERS:
        # Announce return only if it wasn't a command
        afk_data = AFK_USERS.pop(user_id)
        time_afk = get_readable_time(int(time.time() - afk_data["time"]))
        ...
        # We still continue execution to check if the returning user mentioned another AFK user.
        # But we stop reacting to subsequent messages from this user now.
        # Note: If the user sends /afk, that is handled by afk_cmd and this section is skipped due to filters.
    ...
    if message.entities:
        for entity in message.entities:
            ...
            # We skip @username resolution for simplicity and speed, relying mostly on replies/ID mentions.

    for afk_user_id in mentions_to_check:
        # Check only if the user is still AFK (since AFK_USERS might have changed if the sender was AFK)
        if afk_user_id in AFK_USERS and afk_user_id != message.from_user.id:
...
@app.on_message(
    (filters.text & ~filters.command & ~filters.bot) | filters.reply
)
async def general_chat_reply(client, message: Message):
    ...
    if chat_type == enums.ChatType.PRIVATE:
        # Private chat: always reply (as requested: 'private me pura reply')
        should_reply = True
    ...
    if should_reply:
        ...
        # Ignore if the message itself is a command (even if it's a reply)
        if message.text and message.text.startswith('/'):
            return

        # Use get_reply to generate response (applies 80/20 logic)
...
# -------- New Member Join Handler (New Feature: Group Join Message) --------
@app.on_message(filters.new_chat_members & filters.group)
async def welcome_new_members(client, message):
    for user in message.new_chat_members:
        if user.is_self: # Bot itself was added (The requested feature)
            return await message.reply_text(
                "ᴛʜᴀɴᴋs ғᴏʀ ᴀᴅᴅɪɴɢ ᴍᴇ! ❤️\n"
                "ɪ ᴀᴍ ᴀɴ ᴀᴅᴠᴀɴᴄᴇᴅ ᴄʜᴀᴛʙᴏᴛ. ᴜsᴇ /start ᴛᴏ ᴄʜᴇᴄᴋ ᴍʏ ғᴇᴀᴛᴜʀᴇs.",
                parse_mode=enums.ParseMode.MARKDOWN
            )
        else:
            # Optional: Welcome new users logic (can be added here)
            pass
