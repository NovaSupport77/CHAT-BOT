...
# If no specific keyword is found...
        # Fallback to the general daily replies
        return (random.choice(DATA.get("daily", ["Hello 👋"])), False)

# -------- Utility Functions (AFK feature ke liye zaruri) --------

def get_readable_time(seconds: int) -> str:
    """Seconds ko human-readable time string mein convert karta hai."""
    result = ''
    (days, remainder) = divmod(seconds, 86400)
    (hours, remainder) = divmod(remainder, 3600)
    (minutes, seconds) = divmod(remainder, 60)
...
async def is_admin_or_owner(client: Client, chat_id: int, user_id: int) -> bool:
    """Check karta hai ki user group mein admin hai ya bot owner."""
    if user_id == OWNER_ID:
        return True
    try:
...
# 1. New Group Member Handler (Group Add hone par Welcome Message)
# User Request: 𝐓ʜᴀɴᴋs ғᴏʀ ᴀᴅᴅɪɴɢ ᴍᴇ !! ɪ ᴡɪʟʟ ᴋᴇᴇᴘ ᴀᴄᴛɪᴠᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ !!
@app.on_message(filters.new_chat_members & filters.group, group=1)
async def bot_added_to_group_handler(client: Client, message: Message):
    """Bot ke group mein add hone par welcome message bhejta hai."""
    
    # Check karta hai ki bot ko hi add kiya gaya hai
    if client.me.id in [user.id for user in message.new_chat_members]:
        
        # Chat ID save karna (agar aapne save chat logic yaha nahi rakha hai toh)
        chat_id = message.chat.id
        ...
        welcome_text = (
            "𝐓ʜᴀɴᴋs ғᴏʀ ᴀᴅᴅɪɴɢ ᴍᴇ !!\n"
            "ɪ ᴡɪʟʟ ᴋᴇᴇᴘ ᴀᴄᴛɪᴠᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ !!"
        )
...
# 2. /chatbot Command Handler (Chatbot ko On/Off karne ke liye)
@app.on_message(filters.command("chatbot") & filters.group & filters.incoming, group=2)
async def chatbot_toggle_command(client: Client, message: Message):
    """Admin command jo chatbot feature ko enable/disable karta hai."""
    if not await is_admin_or_owner(client, message.chat.id, message.from_user.id):
        await message.reply_text("❌ 𝐎ɴʟʏ 𝐀ᴅᴍɪɴs ᴧɴᴅ 𝐎ᴡɴᴇʀ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴧɴᴅ.")
        return

    try:
        command, status_str = message.text.split(maxsplit=1)
        status_str = status_str.lower()
    except ValueError:
        await message.reply_text("𝐏ʟᴇᴀsᴇ ᴜsᴇ ғᴏʀᴍᴧᴛ: `/chatbot enable` ᴏʀ `/chatbot disable`")
        return

    chat_id = message.chat.id
    if status_str == "enable":
        CHATBOT_STATUS[chat_id] = True
        await message.reply_text("✅ 𝐂ʜᴧᴛʙᴏᴛ ᴇɴᴀʙʟᴇᴅ ʜᴇʀᴇ!")
    elif status_str == "disable":
        CHATBOT_STATUS[chat_id] = False
        await message.reply_text("❌ 𝐂ʜᴧᴛʙᴏᴛ ᴅɪsᴧʙʟᴇᴅ ʜᴇʀᴇ!")
    else:
        await message.reply_text("𝐈ɴᴠᴧʟɪᴅ ᴏᴘᴛɪᴏɴ. 𝐔sᴇ `/chatbot enable` ᴏʀ `/chatbot disable`.")


# 3. Main Message Handler (AFK Check aur Auto-Reply Logic)
@app.on_message(filters.text & filters.group & filters.incoming & ~filters.edited, group=3)
async def afk_and_chatbot_handler(client: Client, message: Message):
    
    # --- 1. AFK Check for mentioned users (Aapke snippet ke anusaar) ---
    if message.mentioned:
        for user_id in AFK_USERS:
            # Check if this message mentions an AFK user
            if any(entity.user and entity.user.id == user_id for entity in message.entities or []):
                afk_data = AFK_USERS[user_id]
                time_afk = get_readable_time(int(time.time() - afk_data["time"]))
                ...
                # Agar message AFK user ne wapas aane ke liye nahi bheja hai
                if message.from_user.id != user_id:
                    await message.reply_text(reply_text, parse_mode=enums.ParseMode.MARKDOWN)
                break 

    # --- 2. Check for AFK return (AFK user ka wapas aana) ---
    if message.from_user.id in AFK_USERS:
        # User koi bhi message bhejkar wapas aa raha hai
        user_id = message.from_user.id
        user_name = message.from_user.first_name
        afk_data = AFK_USERS.pop(user_id)
        time_afk = get_readable_time(int(time.time() - afk_data["time"]))
        ...
        await message.reply_text(
            f"✅ 𝐖ᴇʟᴄᴏᴍᴇ 𝐁ᴧᴄᴋ {user_name}! 𝐘ᴏᴜ ᴡᴇʀᴇ 𝐀ғᴋ ғᴏʀ {time_afk}.",
            parse_mode=enums.ParseMode.MARKDOWN
        )

    # --- 3. Auto-Reply Feature (User Request: 80% chance on non-replies) ---
    chat_id = message.chat.id
    
    # Check karein ki chatbot enabled hai, message reply nahi hai, aur command nahi hai
    is_chatbot_enabled = CHATBOT_STATUS.get(chat_id, True)
    is_not_reply = not message.reply_to_message
    is_not_command = not message.text.startswith("/")

    if is_chatbot_enabled and is_not_reply and is_not_command:
        
        # 80% probability check (random.random() < 0.8)
        if random.random() < 0.8:
            ...
            try:
                if is_sticker:
                    # Sticker reply
                    await message.reply_sticker(sticker=response)
                else:
                    # Text reply
                    await message.reply_text(
                        text=response,
                        quote=True, # Original message ko reply karega
                        parse_mode=enums.ParseMode.MARKDOWN
                    )
            except Exception as e:
                # Agar Sticker bhejte samay error aaye, toh text reply ka attempt karein
                print(f"Error in sending reply: {e}")
                if not is_sticker:
                    pass
                else:
                    await message.reply_text(
                        text=random.choice(DATA.get("daily", ["Hello 👋"])),
                        quote=True
                    )


# -------- OTHER COMMAND HANDLERS (Zaruri commands ke placeholders) --------

@app.on_message(filters.command("start") & (filters.private | filters.group), group=4)
async def start_command(client: Client, message: Message):
    # Yeh aapke existing /start command ke liye placeholder hai
    mention_name = message.from_user.mention
    developer_mention = f"[{DEVELOPER_USERNAME}]({DEVELOPER_HANDLE})"
    
    await message.reply_photo(
        photo=START_PHOTO,
        caption=INTRO_TEXT_TEMPLATE.format(
            mention_name=mention_name,
            developer=developer_mention
        ),
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ 𝐀ᴅᴅ 𝐌ᴇ 𝐓ᴏ 𝐘ᴏᴜʀ 𝐆ʀᴏᴜᴘ ➕", url=f"https://t.me/{client.me.username}?startgroup=true")],
            [InlineKeyboardButton("❤️ 𝐃ᴇᴠᴇʟᴏᴘᴇʀ ❤️", url=SUPPORT_CHAT)],
            [InlineKeyboardButton("📜 𝐇ᴇʟᴘ 𝐌ᴇɴᴜ 📜", callback_data="help_menu")]
        ])
    )

@app.on_message(filters.command("help"), group=5)
async def help_command(client: Client, message: Message):
    # Yeh aapke existing /help command ke liye placeholder hai
    main_help_text = "📜 𝐂ʟɪᴄᴋ ᴧ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ ᴛᴏ sᴇᴇ ᴄᴏᴍᴍᴧɴᴅs."
    await message.reply_text(
        main_help_text,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("👩‍❤️‍👨 𝐂ᴏᴜᴘʟᴇ & 𝐋ᴏᴠᴇ", callback_data="help_couple")],
            [InlineKeyboardButton("🤖 𝐂ʜᴧᴛʙᴏᴛ 𝐂ᴏɴᴛʀᴏʟ", callback_data="help_chatbot")],
            [InlineKeyboardButton("🛠️ 𝐓ᴏᴏʟs & 𝐀ғᴋ", callback_data="help_tools")],
            [InlineKeyboardButton("👥 𝐆ʀᴏᴜᴘ 𝐔ᴛɪʟɪᴛʏ", callback_data="help_group")],
            [InlineKeyboardButton("❌ 𝐂ʟᴏsᴇ ❌", callback_data="close_help")]
        ])
    )
    
@app.on_message(filters.command("afk"), group=6)
async def afk_command(client: Client, message: Message):
    # /afk command logic
    user_id = message.from_user.id
    user_name = message.from_user.first_name
    
    # AFK se wapas aana automatically main handler mein handle ho jayega.
    if user_id in AFK_USERS:
        # Agar user /afk command dobara use karta hai wapas aane ke liye
        AFK_USERS.pop(user_id)
        await message.reply_text(f"✅ 𝐖ᴇʟᴄᴏᴍᴇ 𝐁ᴧᴄᴋ {user_name}! 𝐘ᴏᴜ ᴧʀᴇ ɴᴏ ʟᴏɴɢᴇʀ 𝐀ғᴋ.")
        return
    ...
    await message.reply_text(
        f"💤 {user_name} ɪs 𝐀ғᴋ ɴᴏᴡ.\n📝 𝐑ᴇason: {reason}",
        parse_mode=enums.ParseMode.MARKDOWN
    )
    
@app.on_callback_query(group=7)
async def callback_query_handler(client, callback_query):
    ...
    if data.startswith("help_"):
        key = data.split("_")[1]
        text = HELP_COMMANDS_TEXT_MAP.get(key, "❌ 𝐄ʀʀᴏʀ: 𝐂ᴏᴍᴍᴧɴᴅ ɴᴏᴛ ғᴏᴜɴᴅ.")
        
        # Naye help text ke saath message ko edit karta hai
        await callback_query.message.edit_text(
            text=text,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 𝐁ᴧᴄᴋ 𝐓ᴏ 𝐇ᴇʟᴘ 𝐌ᴇɴᴜ 🔙", callback_data="help")]]),
            parse_mode=enums.ParseMode.MARKDOWN
        )
    elif data == "help":
        # Main help menu par wapas aana
        main_help_text = "📜 𝐂ʟɪᴄᴋ ᴧ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ ᴛᴏ sᴇᴇ ᴄᴏᴍᴍᴧɴᴅs."
        await callback_query.message.edit_text(
            text=main_help_text,
            reply_markup=InlineKeyboardMarkup([
